    ...
    <AppModal onClose={onClose}>
      <div style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div className="modal-body" style={{ padding: '20px 0' }}>
          ...
          {explanation && !loading && (
            <div className="mt-6 glass-subtle p-5" style={{ borderRadius: '16px', background: 'linear-gradient(135deg, #fffaf5 0%, #ffffff 100%)', border: '1px solid #ffedd5' }}>
              <div className="flex justify-between items-center mb-3">
                <h3 style={{ fontWeight: 800, color: '#1e293b', fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div className="text-muted whitespace-pre-wrap text-sm leading-relaxed" style={{ color: '#334155', backgroundColor: 'rgba(255, 255, 255, 0.6)', padding: '16px', borderRadius: '12px', border: '1px solid #f3f4f6' }}>{explanation}</div>

              {/* Modernise Business Logic Section */}
              {moderniseStep === 'none' && (
                <div style={{ marginTop: '28px', paddingTop: '20px', borderTop: '1px dashed #fdba74' }}>
                  <h4 style={{ fontWeight: 800, color: '#ea580c', fontSize: '0.95rem', marginBottom: '8px' }}>⚡ Modernise SAP Business Logic</h4>
                  <p style={{ color: '#64748b', fontSize: '0.8rem', marginBottom: '14px' }}>
                    Select the target architecture to automatically convert this legacy business logic into modern, cloud-compliant frameworks.
                  </p>
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
                    <div style={{ flex: 1, minWidth: '220px' }}>
                      <select
                        ...
                        className="input select"
                        style={{ margin: 0, fontWeight: 600, borderColor: '#fdba74' }}
                      >
                    ...
                    <button
                      ...
                      style={{ whiteSpace: 'nowrap', fontWeight: 700, padding: '10px 20px', background: 'linear-gradient(135deg, #ea580c, #f97316)' }}
                    >
                      Modernise Code
                    </button>
                  </div>
                ...
                <div style={{ marginTop: '28px', paddingTop: '20px', borderTop: '1px dashed #fdba74' }}>
                  <div className="flex justify-between items-center mb-3">
                    <h4 style={{ fontWeight: 800, color: '#ea580c', fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span>🔧</span> Step 1: Refine Prompt Configuration ({moderniseLanguage})
                    </h4>
                    <button
                      type="button"
                  ...
                  {modernisePrompt && (
                    <div className="space-y-4">
                      <div className="input-group">
                        <label className="input-label" style={{ fontWeight: 700, color: '#334155' }}>Optimised Prompt Specifications</label>
                        <textarea
                          ref={promptRef}
                          value={modernisePrompt}
                          onChange={(e) => setModernisePrompt(e.target.value)}
                          className="input font-mono text-xs"
                          style={{ resize: 'none', backgroundColor: '#fcfcfc', borderColor: '#ffd8a8', lineHeight: 1.5 }}
                        />
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <input
                          ...
                          placeholder="e.g., Use UUID for primary key, or include validation checks for VBELN..."
                          className="input"
                          style={{ margin: 0, borderColor: '#ffe8cc' }}
                      ...
                      <button
                        ...
                        className="btn btn-primary w-full"
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '12px 20px', fontWeight: 800, background: 'linear-gradient(135deg, #ea580c, #f97316)' }}
                      >
                        {moderniseLoading ? (
                          <>
                          ...
                          <>
                            ...
                            <span>Generate Modernised Code →</span>
                          </>
                        )}
                      </button>
                    </div>
              ...
              {moderniseStep === 'code' && (
                <div style={{ marginTop: '28px', paddingTop: '20px', borderTop: '1px dashed #fdba74' }}>
                  <div className="flex justify-between items-center mb-3">
                    <h4 style={{ fontWeight: 800, color: '#ea580c', fontSize: '0.95rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span>🚀</span> Step 2: Modernised Code Output ({moderniseLanguage})
                    </h4>
                  </div>

                  {moderniseLoading && (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '50px', gap: '12px' }}>
                  ...
                  {!moderniseLoading && moderniseCode && (
                    <div className="space-y-5">
                      {/* Code container */}
                      <div style={{ position: 'relative', border: '1px solid #ffd8a8', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 4px 12px rgba(234, 88, 12, 0.05)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fff7ed', padding: '8px 16px', borderBottom: '1px solid #ffd8a8' }}>
                          <span style={{ fontSize: '0.72rem', fontWeight: 800, color: '#ea580c', fontFamily: 'monospace', textTransform: 'uppercase' }}>
                          ...
                          <button
                            ...
                            className="btn btn-secondary btn-sm"
                            style={{ padding: '3px 8px', fontSize: '0.7rem', fontWeight: 700, margin: 0 }}
                          >
                            {codeCopied ? '✓ Copied!' : 'Copy Code'}
                          </button>
                        </div>
                        <pre style={{ margin: 0, padding: '16px', backgroundColor: '#0f172a', color: '#f8fafc', overflowX: 'auto', fontSize: '0.8rem', lineHeight: 1.6, fontFamily: "'JetBrains Mono', 'Fira Code', monospace", maxHeight: '320px' }}>
                      ...
                      {moderniseExplanation && (
                        <div style={{ backgroundColor: '#fffaf5', border: '1px solid #ffe8cc', borderRadius: '12px', padding: '16px' }}>
                          <h5 style={{ fontSize: '0.85rem', fontWeight: 800, color: '#ea580c', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <span>💡</span> Technical Architecture & Integration Steps
                          </h5>
                          <div style={{ fontSize: '0.8rem', color: '#475569', whiteSpace: 'pre-wrap', lineHeight: 1.5 }}>{moderniseExplanation}</div>
                        </div>
                      )}

                      {/* Workflow Control Buttons Row */}
                      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginTop: '24px', paddingTop: '16px', borderTop: '1px dashed #fdba74' }}>
                        <button
                          ...
                          className="btn btn-secondary"
                          style={{ flex: 1, minWidth: '130px', fontWeight: 700 }}
                        >
                          ← Back to Prompt
                        </button>
                        <button
                        ...
                        <button
                          ...
                          className="btn btn-primary"
                          style={{ flex: 1.5, minWidth: '180px', fontWeight: 800, background: 'linear-gradient(135deg, #ea580c, #f97316)' }}
                        >
                        ...
                        <button
                          ...
                          className="btn btn-secondary"
                          style={{ flex: 1, minWidth: '140px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
                        >
